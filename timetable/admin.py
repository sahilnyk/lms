...
class TimeTableAdmin(admin.ModelAdmin):
    form = TimeTableForm
    list_display = ('course', 'lesson', 'teacher', 'day_display', 'time_display', 'is_active')
    ...
    def day_display(self, obj):
        colors = {
            'monday': '#2196F3', 'tuesday': '#4CAF50', 'wednesday': '#FF9800',
            'thursday': '#9C27B0', 'friday': '#F44336', 'saturday': '#795548'
        }
        return format_html(
            '<span style="color: {}; font-weight: bold;">{}</span>',
            colors.get(obj.day_of_week, '#000'), obj.get_day_of_week_display()
        )
...
class ClassSessionAdmin(admin.ModelAdmin):
    list_display = ('course', 'lesson_display', 'start_at', 'duration_minutes', 'active', 'notification_status')
    ...
    def notification_status(self, obj):
        try:
            ...
            schedule = NotificationSchedule.objects.filter(content_type=ct, object_id=obj.pk).first()
...
class TimetableNotificationAdmin(admin.ModelAdmin):
    list_display = ('timetable_entry', 'notification_type', 'is_sent', 'sent_at')
